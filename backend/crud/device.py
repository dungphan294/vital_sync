...
def delete_device(db: Session, device_id: str):
    if db.query(Data).filter_by(device_id=device_id).first():
        raise ValueError(
            f"Cannot delete device '{device_id}' — referenced in data records")
    if db.query(UserDevice).filter_by(device_id=device_id).first():
        raise ValueError(
            f"Cannot delete device '{device_id}' — referenced in user_device records")
    db_device = get_device_by_id(db, device_id)
    if not db_device:
        return None
    db.delete(db_device)
